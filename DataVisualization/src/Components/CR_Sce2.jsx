...
const CorrosionRate2 = () => {
  const { fileDataICE } = useContext(FileContext);
   const navigate = useNavigate();

  const xKey = 'Chainage (m)';
  const yKey1 = 'Sce-2 CCR - Worst mmpy';
  const yKey2 = 'Sce-2 CCR - Realistic mmpy';
  const yKey3 = 'Elevation (m)';
  const yKey4 = 'Sce-2 CCR - O2 mmpy';
  const yKey5 = 'Sce-2 CCR - MIC mmpy';
  const yKey6 = 'Sce-2 CCR - General mmpy';
  ...
  const options = {
    ...
    plugins: {
      title: {
        display: true,
        text: `Elevation & Corrosion Rate Scenario 2`,
      },
      legend: {
        display: true,
      },
  ...
  return(
    <div style = {{ maxWidth: '100%',
      ...
      <h2>Corrosion Rate Scenario-2</h2>
      ...
      <div style={{ marginTop: '2rem', marginBottom: '1rem' }}>
        <button style={{fontSize: '18px', cursor: 'pointer'}} onClick={() => {
          ...
          if (input && !isNaN(input)) {
            window.open(`/cr2-chunks?chunk=${input}`, '_blank');
          }
        }}>
