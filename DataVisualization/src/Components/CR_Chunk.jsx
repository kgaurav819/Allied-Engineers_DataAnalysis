...
const CR_Chunk = () => {
  ...
  const filtered = fileDataICE.filter(
    (item) =>
      ...
      item['Sce-1 CCR - General mmpy']&&
      item['Sce-1 CCR - MIC mmpy']&&
      item['Sce-1 CCR - O2 mmpy']&&
      item['Elevation (m)']&&
      !isNaN(item['Chainage (m)']) &&
      !isNaN(item['Sce-1 CCR - Worst mmpy'])&&
      !isNaN(item['Sce-1 CCR - Realistic mmpy'])&&
      !isNaN(item['Sce-1 CCR - General mmpy'])&&
      !isNaN(item['Sce-1 CCR - MIC mmpy'])&&
      !isNaN(item['Sce-1 CCR - O2 mmpy'])&&
  ...
  for (let start = 0; start < maxDist; start += chunkSize) {
    ...
    if (chunk.length > 0) {
      ...
      const generalValues = chunk.map(item => Number(item['Sce-1 CCR - General mmpy']));
      const micValues = chunk.map(item => Number(item['Sce-1 CCR - MIC mmpy']));
      const o2Values = chunk.map(item => Number(item['Sce-1 CCR - O2 mmpy']));
  ...
  return (
    <div style = {{padding: '2rem'}}>
      <h2>Section-wise Charts of Corrosion Rate Scenario-1 for (Every {chunkSize}m)</h2>
      {chunks.map((chunk, i) => (
        ...
        scales: {
          ...
          y: {
            title: {
              display: true,
              text: 'Corrosion Rate Scenario-1'
            }
          },
          y1: {
        display: true,
